...
// 3.2 - Cria uma string com o nome completo do autor
const getNewAuthor = (authorData) => {
  ...
  // 3.2 - serialize o nome dos campos de snake_case para camelCase
  ...
  // 3 - Model com a query para mostrar todos
const findAllAuthors = async () => {
  const [authors] = await connection.execute('SELECT * FROM authors');
  return authors.map(serialize).map(getNewAuthor); // 3.1 - chama as funções serialize e getNewAuthor
};
